...
export function cssWatcher(options = {}) {
  ...
  function scanComponentFolders(componentsDir) {
    try {
      // Get all top-level folders in components directory
      const topLevelEntries = fs.readdirSync(componentsDir, { withFileTypes: true });
      
      // For each top-level folder (like 'elements', 'layouts', etc.)
      for (const topLevelEntry of topLevelEntries) {
        if (topLevelEntry.isDirectory()) {
          ...
          if (fs.existsSync(path.join(topLevelPath, 'index.astro'))) {
            if (config.verbose) {
              console.log(`[CSS Watcher] Found component in top-level folder: ${topLevelRelativePath}`);
            }
            scanComponentDir(topLevelPath);
          }
          
          // Get all component folders within this top-level folder
          const componentEntries = fs.readdirSync(topLevelPath, { withFileTypes: true });
  ...
  return {
    ...
    configureServer(server) {
      ...
      try {
        ...
        console.log(`[CSS Watcher] Found ${cssToComponentMap.size} CSS files across ${scannedDirs.size} component directories`);
        ...
        if (config.verbose && cssToComponentMap.size > 0) {
          ...
          for (const [cssFile, componentFile] of cssToComponentMap.entries()) {
            ...
            const relativeComponentPath = componentFile.split('packages/components/')[1] || componentFile;
